//+------------------------------------------------------------------+
//|                             NewArch_SR_MeanReverting_v3b.mq5    |
//|                                  Copyright 2026, Antigravity AI  |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Antigravity AI"
#property link "https://www.mql5.com"
#property version "3.00b"
#property strict
...
// CONFIG
// ─────────────────────────────────────────────
input group "SR v3b - REST API Settings"
...
input group "Strategy Settings (FOLLOW TREND ONLY)"
...
input uint InpMagic = 123463;             // Magic Number
...
void ParseAndLoadLevels(string data, string source) {
  ...
  Print("v3b Follow Trend | Source: ", source, " | Loaded ", ArraySize(ActiveLevels), " levels.");
  DrawSRLines();
}
...
void DrawLine(double price, int idx) {
    ...
    ObjectSetInteger(0, name, OBJPROP_COLOR, clrOrangeRed); // Different color for distinction
    ...
    ObjectSetString(0, lbl, OBJPROP_TEXT, " v3b_SR " + DoubleToString(price, _Digits));
...
void HandleSignals() {
  ...
  // VERSION v3b FOLLOW TREND LOGIC:
  // Buy if Price just touched SR and Closed ABOVE, AND last candle must be GREEN (Bullish)
  if (close_0 > focus_sr) { // BUY Signal
    if (close_1 < open_1) { // Last candle was Bearish -> Not Follow Trend
       Print("v3b: SKIP BUY - Not follow trend (Last candle Bearish)");
       return;
    }
    double tp_price = 0;
    if (GetNearestSR(close_0, true, tp_price)) {
      ...
      m_trade.Buy(InpLotSize, _Symbol, close_0, sl_price, tp_price, "v3b FollowTrend Long");
    }
  } else { // SELL Signal
    if (close_1 > open_1) { // Last candle was Bullish -> Not Follow Trend
       Print("v3b: SKIP SELL - Not follow trend (Last candle Bullish)");
       return;
    }
    double tp_price = 0;
    if (GetNearestSR(close_0, false, tp_price)) {
      ...
      double sl_price = close_0 + (tp_dist / InpRatio);
      m_trade.Sell(InpLotSize, _Symbol, close_0, sl_price, tp_price, "v3b FollowTrend Short");
    }
  }
}
...
void HandleOvertime() {
  ...
  if (TimeCurrent() - open_time >= (datetime)InpOvertime * 3600) {
    m_trade.PositionClose(ticket, "Force Closed - v3b Overtime");
  }
}
